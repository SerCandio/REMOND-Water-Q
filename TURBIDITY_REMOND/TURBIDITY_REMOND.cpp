...
float REMOND_TURB_Get_Temperature(ModbusMaster& node){
  if(!turbiditYUpdateValues(node))
    return NAN;
  return _turbiditYcurrentValues.temperature; 
}

/*
 * Funcion que retorna el valor de la corriente de salida basado en turbidez
 * 
 *    Parametros : 
 *        node        :  Objeto Modbus asigando al sensor  (&node)
 */
float REMOND_TURB_Get_Current_Output(ModbusMaster& node){
  if(!turbiditYUpdateValues(node))
    return NAN;
  return _turbiditYcurrentValues.currentOut;  
}
...
uint8_t REMOND_TURB_Get_Warning(ModbusMaster& node){
  if(!turbiditYUpdateValues(node))
    return NAN;
  return _turbiditYcurrentValues.warnings;    
}
...
static bool turbiditYUpdateValues(ModbusMaster& node){
  ...
  if( (unsigned long)(millis() - ( _lastRead6) )  >  UPDATE_TIME_TURBIDITY){
  ...
  if(result == node.ku8MBSuccess)
  {
    ...
    // Captura de las medidas ....
     ...
     _turbiditYcurrentValues.temperature=*((float*) &registerData );    // Cast HEX float to float value

                   //AB                                   CD
     registerData=( ( response[5] ) << 16 ) & 0xFFFF0000 | response[4] ;
     _turbiditYcurrentValues.currentOut=*((float*) &registerData );    // Cast HEX float to float value

     _turbiditYcurrentValues.warnings= (uint8_t) ( response[6]) & 0x00FF ; // As warnings go from 00 to 04 only !!
